...
import com.eshop.modal.entry.api.marketplace.supply.vo.AeAscpPoErpPageQueryDataVO;
import com.eshop.modal.entry.api.marketplace.supply.vo.AeAscpPoErpPageQueryItemDataVO;
import com.eshop.modal.entry.api.marketplace.supply.vo.AeAscpPoErpPageQueryItemVO;
import com.eshop.modal.entry.api.marketplace.supply.vo.AeAscpPoErpPageQueryVO;
...
import com.eshop.util.platform.api.service.supply.aliexpress.dto.AeAscpPoErpPageQueryDTO;
import com.eshop.util.platform.api.service.supply.aliexpress.dto.AeAscpPoErpPageQueryItemDTO;
 ...
 * @author xxs
 * @date 2024/9/05
 * @Description: 速卖通全托管仓发备货单Client 实现类
 * @Version 1.0
 */
@StrategyKey(ConstantPlatform.AE)
@Service
@Slf4j
public class AliExpressAscpCfOrderClientImpl implements AliExpressAscpCfOrderClient {
    ...
    public PageUtil getSyncOrderListCount(PlatformSyncDTO dto, OrderSyncDTO osDTO, Integer pageNo, Integer pageSize) throws Exception {
        ...
        AeAscpPoErpPageQueryDTO pageQueryReqDTO = getCfListReqDTO(osDTO, pageNo, pageSize);
        pageQueryReqDTO.setChannel_user_id(Long.valueOf(sellRelationQuery(acDTO).getChannel_seller_id()));
        AeAscpPoErpPageQueryVO respVO = aeAscpCallService.ascpPoErpPageQuery(acDTO, pageQueryReqDTO);
        Integer total_item = respVO.getTotal_count().intValue();
...
//        }
    ...
    public PlatformSyncListResponseVO<PlatformSyncInfoResponseVO<TbOrder>> getAddSyncOrderList(PlatformSyncDTO dto, OrderSyncDTO osDTO, Integer pageNo, Integer pageSize, PlatformPageCursorDto cursorDto) throws Exception {
        ...
        List<AeAscpPoErpPageQueryDataVO> orderListVOS = respVO.getData();
        JSONObject respJson = JSONObject.parseObject(respBody);
    ...
    public TbOrderInfo getAddOrderDetailSync(PlatformSyncDTO psDTO, PlatformSyncInfoResponseVO<TbOrder> po) throws Exception {
        ...
        AeAscpPoErpPageQueryItemVO baseJson = getBaseJson(shopDO, orderNo);
...
//        AeAscpPoPageQueryDataVO ascpPoData1 = mapper.readValue(jsonObj.toString(), AeAscpPoPageQueryDataVO.class);
        AeAscpPoErpPageQueryDataVO ascpPoData = mapper.readValue(jsonObj.toString(), AeAscpPoErpPageQueryDataVO.class);
        List<AeAscpPoErpPageQueryItemDataVO> dataList = baseJson.getData();
...
//        Integer platformOrderStatus = Integer.valueOf(ascpPoData.getBiz_status());

//        String platformOrderStatusStr = aeStatusUtils.getPlatformOrderStatusStr(orderStatusStr);
//        String abnormalCause = aeStatusUtils.getAbnormalCause(orderStatusStr);
//        List<Long> orderTypeMiddles = aeStatusUtils.getOrderTypeMiddles(orderStatusStr, orderInfo, orderEndReason, true);

//        order.setPlatformOrderStatus(aeStatusUtils.getAeAscpPlatformOrderStatus(platformOrderStatus));
...
//        }
    ...
    public TbOrderInfo getUpdateOrderDetailSync(PlatformSyncDTO psDTO, PlatformSyncInfoResponseVO<TbOrder> po) throws Exception {
        ...
        //获取订单详情
        AeAscpPoErpPageQueryItemVO respVO = getBaseJson(shopDO, orderNo);
        List<AeAscpPoErpPageQueryItemDataVO> target = respVO.getData();
        ...
        AeAscpPoErpPageQueryDataVO ascpPoData = mapper.readValue(jsonObj.toString(), AeAscpPoErpPageQueryDataVO.class);
        //平台订单状态 原生
        Integer orderStatusStr = Integer.valueOf(ascpPoData.getBiz_status());
        return getUpOrderInfoLevel1(psDTO, orderNo, orderStatusStr, target);
    }

    private TbOrderInfo getUpOrderInfoLevel1(PlatformSyncDTO psDTO, String orderNo, Integer orderStatusStr, List<AeAscpPoErpPageQueryItemDataVO> target) {
    ...
    private TbOrderInfo getUpOrderInfoLevel2(PlatformSyncDTO psDTO, TbOrder order, Integer orderStatus, List<AeAscpPoErpPageQueryItemDataVO> target) {
...
//        List<Long> orderTypeMiddles = aeStatusUtils.getOrderTypeMiddles(orderStatus, orderInfo, orderEndReason, false);
        ...
        orderInfo.setPlatformOrderStatus(orderStatus);
//        orderInfo.setAbnormalCause(abnormalCause);
//        orderInfo.setOrderTypeMiddles(orderTypeMiddles);
        //订单详情原始数据
        if (target != null) {
...
//        queryReqDTO.setData_range();
    ...
    private AeAscpPoErpPageQueryDTO getCfListReqDTO(OrderSyncDTO osDto, Integer pageNo, Integer pageSize) {
        if (osDto.getStartDate() == null) {
            throw new RuntimeException("时间范围开始时间不能为空!");
        }
        ...
        AeAscpPoErpPageQueryDTO queryReqDTO = new AeAscpPoErpPageQueryDTO();
        // 采购单号
        queryReqDTO.setMerchant_code("AECHOICE");
        // 单据状态 10:待确认 15:已确认 17:待发货 20:待收货 21:已到仓 30:部分收货 40:收货完成 -99:已取消,不传则返回所有状态的采购单
//        queryReqDTO.setStatus();
        // 渠道seller id （可以在这个API中查询：global.seller.relation.query）， 请使用 business_type = ONE_STOP_SERVICE 的全托管店铺 channel_seller_id, choice2.0(当bizType=288000时)场景下可以不传该参数)
//        queryReqDTO.setChannel_user_id();
...
//        queryReqDTO.setData_range();
        ...
        // 开始时间  "yyyy-MM-dd HH:mm:ss"
        queryReqDTO.setGmt_create_begin(startTimeStr);
        // 结束时间  "yyyy-MM-dd HH:mm:ss"
        queryReqDTO.setGmt_create_end(endTimeStr);
        // 当前页
        queryReqDTO.setPage_index(pageNo);
        // 分页大小
        queryReqDTO.setPage_size(pageSize);
    ...
    private List<PlatformSyncInfoResponseVO<TbOrder>> getAeOrderList(List<AeAscpPoErpPageQueryDataVO> orderListVOS) {
        List<PlatformSyncInfoResponseVO<TbOrder>> orderList = new ArrayList<>();
        for (AeAscpPoErpPageQueryDataVO orderListVO : orderListVOS) {
            //order订单VO json
    ...
    private AeAscpPoErpPageQueryItemVO getBaseJson(TbShop shopDO, String orderNo) throws Exception {
        String order_id = null;
        if (ObjectUtil.isNotEmpty(orderNo)) {
            order_id = orderNo;
        }
        AeAppClientDTO acDTO = platformAppClientUtils.getAeAppClientDTO(shopDO);
        AeAscpPoErpPageQueryItemDTO reqDTO = new AeAscpPoErpPageQueryItemDTO();
        reqDTO.setPurchase_order_no(order_id);
        reqDTO.setPage_size(100);
        reqDTO.setMerchant_code("AECHOICE");
        reqDTO.setPage_index(1);
        reqDTO.setChannel_user_id(Long.valueOf(shopDO.getPlatformShopId()));
        //api请求
        AeAscpPoErpPageQueryItemVO odRespVO = aeAscpCallService.ascpPoErpPageQueryItem(acDTO, reqDTO);
        return odRespVO;
    }
     ...
     * @return
     */
    private TbOrder handleAeOrderDetail(AeAscpPoErpPageQueryDataVO ascpPageDataVO, TbOrder orderOld, AeAscpPoErpPageQueryItemVO odVO) {
        //拷贝orderOld
        TbOrder order = new TbOrder();
        ...
        Long gmtCreate = (Long) ascpPageDataVO.getGmt_create();
        Long gmtExpiration = (Long) ascpPageDataVO.getGmt_terminate();
//        String tradeOrderId = ascpPageDataVO.getTrade_order_id();
        ...
        order.setOrderNo(orderNo);
        order.setPlatformOrderStatus(Integer.valueOf(ascpPageDataVO.getBiz_status().toString()));
    ...
    private TbOrderBuyerInfo getOrderBuyerInfo(String orderNo, AeAscpPoErpPageQueryDataVO ascpPoData) {

        TbOrderBuyerInfo orderBuyer = new TbOrderBuyerInfo();
        //buyer_user_id 此订单买家的用户ID  交易主单号先作为买家id
        String buyerId = ascpPoData.getPurchase_order_no();
    ...
    private TbOrderCostDetail getOrderCostDetail(String orderNo, AeAscpPoErpPageQueryDataVO ascpPoData) {
        return aeOrderUtils.getCfOrderCostDetail(orderNo, ascpPoData);
    }

    private List<TbWaybill> getWaybills(String orderNo, AeAscpPoErpPageQueryDataVO odVO) {
//        //获取物流方式
...
//        }
    ...
    private TbWaybillSite getWaybillSite(String orderNo, AeAscpPoErpPageQueryDataVO odVO, TbShop shopDO) {
...
//        }
//        waybillSite.setWaybillId(111111L);
    ...
    private List<AeChoiceProductDetailVO> getAeProductInfo(TbShop shopDO, List<AeAscpPoErpPageQueryItemDataVO> dataList) throws Exception {
...
//        for (AeAscpPoErpPageQueryItemDataVO aeAscpPoItemDataRespVO : dataList) {
//            aeProductChoiceQueryReqDTO.setProduct_id(aeAscpPoItemDataRespVO.getSc_item_id());
...
//        }
    ...
    private List<TbOrderProducts> getOrderProductsList(String orderNo, AeAscpPoErpPageQueryItemVO odVO, List<AeChoiceProductDetailVO> aeChoiceProductDetailVOS) throws Exception {
        List<AeAscpPoErpPageQueryItemDataVO> coVOS = odVO.getData();
...
//        }
        for (AeAscpPoErpPageQueryItemDataVO coVO : coVOS) {
            //product_id	Number	商品ID
            Long product_id = (Long) coVO.getSc_item_id();
            //条形码
            String scItemCode = coVO.getSc_item_code();
            //product_name	String	商品名称
            String product_name = coVO.getTitle();
            //product_count	Number	商品数量
            Integer product_count = (Integer) coVO.getQuantity();
            //sku	String	商品SKU  TODO 根据第三方供应链信息里的skuid 查询绑定关系 找到我们平台的sku信息，完善sku相关信息 这里先不管，订单没有返回skuCode
            // String sku = coVO.getSku_code();
            // 未税采购单价，单位元 -原价
            String noTaxPurchasePriceDec = "0";
            // 三方平台图片url
//            AeChoiceProductDetailVO aeChoiceProductDetailVOFilter = aeChoiceProductDetailVOS.stream().filter(detail -> detail.getOne_stop_service_product_dto().getProduct_info_dto().getProduct_id().equals(product_id)).findFirst().orElse(null);
//            if (ObjectUtil.isEmpty(aeChoiceProductDetailVOFilter)) {
//                throw new Exception("获取速卖通订单里商品详情信息失败");
//            }

//            String skuImg = coVO.getsku_img();
//            String skuId = coVO.getSku_id();
            JSONObject object = JSONObject.parseObject(coVO.getExt_fields());
            String productIds = object.getString("productIdList");
            String pictureUrl = object.getString("pictureUrl");

            String skuImg = pictureUrl;
            String skuId = productIds;
            ...
            orderProducts.setPlatformSkuIndex(product_id);
            // 存储货品编码好了
//            orderProducts.setPlatformSkuId(Long.valueOf(scItemCode));
...
//            List<AeChoiceProductSkuVO> aeChoiceProductSkuVO = skuVOS.stream().filter(sku -> (ObjectUtil.isNotEmpty(sku.getSku_id()) && sku.getSku_id().equals(Long.valueOf(skuId)))).collect(Collectors.toList());
//            if (ObjectUtil.isNotEmpty(aeChoiceProductSkuVO)) {
//                orderProducts.setShopSku(aeChoiceProductSkuVO.get(0).getSku_code());
//            }
            orderProducts.setShopSku(coVO.getSc_item_code());
...
//            orderProducts.setPlatformSkuId(Long.valueOf(skuId.split(",")[0]));
