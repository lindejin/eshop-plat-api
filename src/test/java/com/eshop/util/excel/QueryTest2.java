...
public class QueryTest2 {
    ...
    public List<TbOrderProducts> getOpList(Set<String> orderNos) {
        ...
        List<CompletableFuture<List<TbOrderProducts>>> futures = parcelNoChunks.stream()
                .map(chunk -> CompletableFuture.supplyAsync(() -> {
                    try {
                        LambdaQueryWrapper<TbOrderProducts> lqWrapper = Wrappers.<TbOrderProducts>lambdaQuery();
                        lqWrapper.in(TbOrderProducts::getOrderNo, orderNos);
                        List<TbOrderProducts> tempList = iTbOrderProductsService.list(lqWrapper);
                ...
                .map(future -> future.thenApply(list -> list.stream().map(TbOrderProducts.class::cast).collect(Collectors.toList())))
                .collect(Collectors.toList());

        List<TbOrderProducts> results = futures.stream()
                .map(CompletableFuture::join) // 等待所有任务完成
                .filter(p -> ObjectUtil.isNotEmpty(p))
                .flatMap(Collection::stream)
                .collect(Collectors.toList());

        List<TbOrderProducts> ppList = results.stream().filter(p -> ObjectUtil.isNotEmpty(p)).collect(Collectors.toList());
        return ppList;
    }
