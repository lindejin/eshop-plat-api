...
//            executor.shutdown(); // 关闭线程池
    ...
    private List<TbOrderProducts> getOpList() {
        // 创建查询条件
        LambdaQueryWrapper<TbOrderProducts> opWrapper = Wrappers.<TbOrderProducts>lambdaQuery();
        opWrapper.isNotNull(TbOrderProducts::getProductSku);
        opWrapper.isNotNull(TbOrderProducts::getSkuImgId);
        // 执行查询
        List<TbOrderProducts> nullRecords = iTbOrderProductsService.list(opWrapper);
        System.out.println(nullRecords.size());
        return nullRecords;
    }
