...
# 批量删除
DELETE FROM `eshop_log`.`tb_order_err_log`
WHERE `err_describe` LIKE '(add)(list-filter)(biz:order) filtered result is empty!%'
    LIMIT 100000;

# 批量删除
DELETE FROM `eshop_log`.`tb_order_err_log`
WHERE `err_describe` LIKE '(update)(list-filter)(biz:order) filtered result is empty!%'
    LIMIT 100000;
